...
class Triax:
    ...
    def flush_buffer(self):
        ...
        if DEBUGMODE:
            print('Flush... (', n_bytes, ') bytes:', end=' ')
        if n_bytes > 0:
            r_bytes = self.triax.read_bytes(n_bytes)
            if DEBUGMODE:
                print(r_bytes)
            n_bytes = 0
        elif DEBUGMODE:
    ...
    def open(self):
        ...
        self.triax = rm.open_resource('GPIB0::1::INSTR')
        ...
        while n_bytes <= 0:
            ...
            if restart == 3:
                self.triax.close()
                print('Could not establish communication with Monochromator')
                return
            if DEBUGMODE:
    ...
    def set_motor_pos(self, pos):
        ...
        # it checks if the new position is inside limits
    ...
    def move_motor(self, move_steps):
        '''Moves motor position. The function waits for the motor to stop to
        continue.
        Param:
        ...
        Returns:
        true if confirmation "o" is received from triax'''
        ##habría que tener en cuenta el backlash para movimientos
        # negativos
    ...
    def swipe_motor(self, lambda_i, lambda_f, stepsize):
        '''Swipes the motor from "lambda_i" to "lambda_f" with steps of
        size "stepsize".
        Param:
        lambda_i (int): Initial position in nm 300-1100 nm
        lambda_f (int): Final position in nm 300-1100 nm
        stepsize (int): Step size in nm 5-50 nm'''
        if lambda_i < 300 or lambda_i > 1100 or lambda_f < 300 or lambda_f > 1100:
            print('Error: Values off limits. Swipe range: 300-1100 nm')
            return
        if stepsize < 5 or stepsize > 50:
            print('Error: Stepsize has to be between 5-50 nm')
            return
    ...
    def motor_busy_check(self):
        '''Polls Triax to check if the motor (also slit motor) movement
